...
function compile(project, tsOptions, lintOptions, force) {
    var results = {};
    ...
    _.each(emittedFiles, function (file) {
        ...
        if (lintOptions) {
            var linter = new Linter(fileName, '', lintConfig, program);
...
function compileProject(projectName, tsPublishConfigPath, force) {
    var projects = cache_1.parseTsPublishConfig(tsPublishConfigPath);
    if (!projects) {
        var configFile = tsPublishConfigPath + ".json";
    ...
    var lintOptions = cache_1.getConfig('tslint');
    var results = compile(project, project.compilerOptions, lintOptions, force);
    var output = {
        results: results,
        numMessages: 0,
        numErrors: 0,
